...
- Both deferred bigger questions are now resolved (same day, follow-up research) — see `CLAUDE.md`. **unkey-auth**: the original "duplication" framing was wrong; `TenantRegistry` solves a different problem (per-tenant data isolation) that nothing else in the fleet has yet either, and stays as-is — don't replace it. **`WorkflowExecutor` vs. `baselayer/core_loop`**: keep this repo's `WorkflowExecutor`; baselayer's engine cannot call an external HTTP service at all today (its `webhook` step type is an unimplemented stub), has no `$steps.x.y` templating, and has no `tenant_id` on its own workflow models — migrating would mean rebuilding this repo's load-bearing capabilities inside a system with weaker tenant isolation than what's here now.
